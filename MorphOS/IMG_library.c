...
STATIC CONST TEXT libname[] = "sdl2_image.library";
...
STATIC VOID sort_ctdt(struct SDL2ImageLibrary *LibBase)
{
...
static int init_libs(struct SDL2ImageLibrary *base, struct ExecBase *SysBase)
{
...
struct Library	*LIB_Init(struct SDL2ImageLibrary *LibBase, BPTR SegList, struct ExecBase *sysBase)
{
	register char *r13;
...
static BPTR DeleteLib(struct SDL2ImageLibrary *LibBase, struct ExecBase *SysBase)
{
	BPTR SegList = 0;
...
{
	struct SDL2ImageLibrary *LibBase = (APTR)REG_A6;
...
BPTR LIB_Close(void)
{
	...
	if (LibBase->Parent)
	{
		struct SDL2ImageLibrary *ChildBase = LibBase;
...
{
	struct SDL2ImageLibrary *LibBase = (APTR)REG_A6;
	struct SDL2ImageLibrary	*newbase, *childbase;
	struct ExecBase *SysBase = LibBase->MySysBase;
	...
	ForeachNode(&LibBase->TaskContext.TaskList, ChildNode)
	{
		...
		{
			/* Yep, return it */
			childbase = (APTR)(((ULONG)ChildNode) - offsetof(struct SDL2ImageLibrary, TaskContext.TaskNode.Node));
...
{
	sizeof(struct SDL2ImageLibrary),
	(size_t)FuncTable,
	0,
	(size_t)LIB_Init
};
